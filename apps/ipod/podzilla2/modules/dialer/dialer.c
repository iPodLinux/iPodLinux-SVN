...
#ifdef __linux__
static pz_dsp_st dialer_dspz;
#else
static int dialer_dspz;
#endif
...
void dtmf_play( pz_dsp_st *dspz, char key )
{
	int bp;
	int f1p = 0;
	int f2p = 0;
...
#ifdef __linux__
		pz_dsp_write( dspz, buf, nsamps*sizeof(short));
#endif
...
static void dialer_press_button(int pos) {
	dtmf_play( &dialer_dspz, dialerpad[pos][0] );
}
...
#ifdef __linux__
        ret = pz_dsp_open(&dialer_dspz, PZ_DSP_LINEOUT);
#endif
        if (ret < 0) {
                pz_perror("/dev/dsp");
                return;
...
#ifdef __linux__
	ret = pz_dsp_setup(&dialer_dspz, 1, 44100);
#endif
...
void dialer_dial( char * number )
{
	...
	while ( number[x] != '\0' )
	{
		dtmf_play( &dialer_dspz, number[x++] );
	}
...
PZ_SIMPLE_MOD_GROUP ("dialer", new_dialer_window, "/Extras/Utilities/Dialer", "Desk" )
