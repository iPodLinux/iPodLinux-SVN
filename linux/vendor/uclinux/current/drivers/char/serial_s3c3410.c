/*
 * serial_s3c3410.c: Serial port driver for the Samsung S3C3410X builtin UART
 *
 ...
 * 		       	by Michael Leslie    <www.ArcturusNetworks.com>
 *
...
#define RX_FIFO_LEVEL		4	/* 	2 or 4 or 6 or 8	*/
					/* Transfer FIFO trigger level	*/
#define TX_FIFO_LEVEL		4	/*	0 or 2 or 4 or 6	*/
...
static struct s3c3410_serial s3c3410_info;
...
#if defined(CONFIG_SERIAL_S3C3410_CONSOLE)
static int console_number =  0;
#endif
	
DECLARE_TASK_QUEUE(tq_s3c3410_serial);
...
static void change_speed	(struct s3c3410_serial *info);
...
static _INLINE_ void xmit_char		(struct s3c3410_serial *info, char ch);
static _INLINE_ void rs_sched_event	(struct s3c3410_serial *info, int event);
static _INLINE_ void uart_speed		(struct s3c3410_serial *info, unsigned int cflag);
static _INLINE_ void handle_status 	(struct s3c3410_serial *info, unsigned int status);
static _INLINE_ void fifo_reset		(void);
static _INLINE_ void fifo_init		(void);

static void set_ints_mode	(struct s3c3410_serial *info, int yes);
...
static void handle_termios_tcsets(struct termios *ptermios, struct s3c3410_serial *pinfo);
...
static _INLINE_ void tx_start(int ints)
{
	if (ints) enable_irq(S3C3410X_INTERRUPT_UTX);
}
...
static _INLINE_ void rx_start(int ints)
{
	if (ints) {
		...
		enable_irq(S3C3410X_INTERRUPT_UERR);
	}
}
...
static _INLINE_ void rx_stop(void)
{
	...
	disable_irq(S3C3410X_INTERRUPT_UERR);
}
...
static void set_ints_mode(struct s3c3410_serial *info, int yes)
{
	info->use_ints = yes;
	if (yes) {
		s3c3410_unmask_irq(S3C3410X_INTERRUPT_UTX);
		s3c3410_unmask_irq(S3C3410X_INTERRUPT_URX);
		s3c3410_unmask_irq(S3C3410X_INTERRUPT_UERR);
	} else {	
		s3c3410_mask_irq(S3C3410X_INTERRUPT_UERR);
		s3c3410_mask_irq(S3C3410X_INTERRUPT_URX);
		s3c3410_mask_irq(S3C3410X_INTERRUPT_UTX);
	}
}

static void hw_flow_control (int cmd)
{
	/* the SMDK40100 board does not have hw flow control */
}

static _INLINE_ void fifo_reset()
{
	u_int8_t ufcon = inb(S3C3410X_UART_BASE+S3C3410X_UFCON);
	ufcon |= UFCON_RFR | UFCON_TFR;
	outb(ufcon, S3C3410X_UART_BASE+S3C3410X_UFCON);
}
...
static _INLINE_ void fifo_init()
{
	...
	switch (RX_FIFO_LEVEL) {
		case 2 :	ufcon |= UFCON_RFTL_2; break;
		case 4 :	ufcon |= UFCON_RFTL_4; break;
		case 6 :	ufcon |= UFCON_RFTL_6; break;
		case 8 :	ufcon |= UFCON_RFTL_8; break;
	}
	switch (TX_FIFO_LEVEL) {
		case 0 :	ufcon |= UFCON_TFTL_0; break;
		case 2 :	ufcon |= UFCON_TFTL_2; break;
		case 4 :	ufcon |= UFCON_TFTL_4; break;
		case 6 :	ufcon |= UFCON_TFTL_6; break;
	}
	outb(ufcon, S3C3410X_UART_BASE+S3C3410X_UFCON);
}

static inline int serial_paranoia_check(struct s3c3410_serial *info,
					dev_t device, const char *routine)
{
	...
	static const char *badinfo =
		"Warning: null s3c3410_serial struct for (%d, %d) in %s\n";
...
static inline void set_dtr(struct s3c3410_serial *info, int set)
{
	/* the SMDK40100 board does not have hw flow control */
}
...
static inline void set_rts(struct s3c3410_serial *info, int set)
{
	/* the SMDK40100 board does not have hw flow control */
}

/* Reads value of serial status signals */
static unsigned int get_status(struct s3c3410_serial *info)
{
	/* the SMDK40100 board does not have hw flow control */
	return 0;
}
...
static void rs_put_char(struct s3c3410_serial *info, char ch)
{
    unsigned long flags = 0;
...
static void batten_down_hatches(void)
{
	...
	if ((((unsigned long) linux_dbvec) >= DEBUG_FIRSTVADDR) &&
		...
		panic("s3c3410_serial: batten_down_hatches");
	return;
#endif
}
...
{
	info->event |= 1 << event;
	queue_task_irq_off(&info->tqueue, &tq_s3c3410_serial);
	mark_bh(SERIAL_BH);
}
...
#define USTAT_ERROR  (USTAT_OE | USTAT_PE | USTAT_FE | USTAT_BD)
#define UFSTAT_ERROR (UFSTAT_RFF)
...
{
	struct s3c3410_serial *info = (struct s3c3410_serial *) private_;
...
// ###	outw(divisor, S3C3410X_UART_BASE+S3C3410X_UBRDIV);
}

static int startup(struct s3c3410_serial *info)
{
	unsigned long flags;
 ...
 */
static void shutdown(struct s3c3410_serial *info)
{
	unsigned long flags;
...
static void change_speed(struct s3c3410_serial *info)
{
	...
	ulcon = 0x00;
	switch  (cflag & CSIZE) {
		case CS5: ulcon |= ULCON_WL_5; break;
		case CS6: ulcon |= ULCON_WL_6; break;
		case CS7: ulcon |= ULCON_WL_7; break;
		case CS8: ulcon |= ULCON_WL_8; break;
	}
	if (cflag & PARENB)
		ulcon |= (cflag & PARODD) ? ULCON_PMD_ODD : ULCON_PMD_EVEN;
	if (cflag & CSTOPB)
		ulcon |= ULCON_SB;
	outb(ulcon, S3C3410X_UART_BASE+S3C3410X_ULCON);

	/* UART control register setup */
	ucon = 0;
	ucon |= UCON_RM_IRQ_POLL; /* receive mode: IRQ or polling */
	ucon |= UCON_TM_IRQ_POLL; /* transmit mode: IRQ or polling */
	ucon |= UCON_RSIE;        /* Rx status interrupt enable */
	ucon |= UCON_RXTOEL;      /* Rx timeout enable (5 bits) */
	outb(ucon, S3C3410X_UART_BASE+S3C3410X_UCON);
...
static inline void wait_for_xmitr(void)
{
	...
	do {
		...
		if (--tmout == 0)
			break;
	} while((status & (USTAT_TFE | USTAT_TSE)) != (USTAT_TFE | USTAT_TSE));
}
...
static _INLINE_ void xmit_char(struct s3c3410_serial *info, char ch)
{
	wait_for_xmitr();	
	tx_delay();
	outb(ch, S3C3410X_UART_BASE+S3C3410X_UTXH_B);
}
...
static int get_serial_info(struct s3c3410_serial *info,
...
static int set_serial_info(struct s3c3410_serial *info,
			   struct serial_struct *new_info)
{
	struct serial_struct new_serial;
	struct s3c3410_serial old_info;
	int retval = 0;
 ...
 */
static int get_lsr_info(struct s3c3410_serial *info, unsigned int *value)
{
 ...
 */
static void send_break(struct s3c3410_serial *info, int duration)
{
	u_int8_t ucon;
	...
	ucon = inb(S3C3410X_UART_BASE+S3C3410X_UCON);
	ucon |= UCON_SBS;
	outb(ucon, S3C3410X_UART_BASE+S3C3410X_UCON);
					...
					unsigned int cmd, unsigned long arg)
{
	...
	case TIOCSERGSTRUCT:
		error = verify_area(VERIFY_WRITE, (void *) arg,
			sizeof(struct s3c3410_serial));
		if (error)
			return error;
		memcpy_tofs((struct s3c3410_serial *) arg, info,
			sizeof(struct s3c3410_serial));
		return 0;
...
static struct irqaction irq_uartTx =
	{ rs_interruptTx,  0, 0, "S3C3410_UART_TX",  NULL, NULL };
static struct irqaction irq_uartRx =
	{ rs_interruptRx,  0, 0, "S3C3410_UART_RX",  NULL, NULL };
static struct irqaction irq_uartErr =
	{ rs_interruptErr, 0, 0, "S3C3410_UART_ERR", NULL, NULL };
	
extern int setup_arm_irq(int, struct irqaction *);

static void interrupts_init(void)
{
	setup_arm_irq(S3C3410X_INTERRUPT_UTX,  &irq_uartTx);
	setup_arm_irq(S3C3410X_INTERRUPT_URX,  &irq_uartRx);
	setup_arm_irq(S3C3410X_INTERRUPT_UERR, &irq_uartErr);
}

static void show_serial_version(void)
{
	printk("Samsung S3C3410X UART driver version 0.1 <thomas.eschenbacher@sympat.de>\n");
}

/* rs_init inits the driver */
static int __init rs_s3c3410_init(void)
{
	int flags, i;
	struct s3c3410_serial *info;
	...
	for (i = 0; i < USART_CNT; i++) {
		...
		info->tty = 0;
		info->irqmask = S3C3410X_INTERRUPT_UTX || 
		                S3C3410X_INTERRUPT_URX || 
		                S3C3410X_INTERRUPT_UERR;
		info->irq = S3C3410X_INTERRUPT_UTX;
		info->port = (i) ? 1 : 2;
		info->line = i;
		...
		printk("%s%d (irq = %d)", serial_driver.name, info->line,
			   info->irq);
		printk(" is a builtin Samsung S3C3410 UART\n");
	}
...
static kdev_t s3c3410_console_device(struct console *);
static int    s3c3410_console_setup(struct console *, char *);
static void   s3c3410_console_write (struct console *, const char *, unsigned int );
static int    s3c3410_console_initialized = 0;

static struct console s3c3410_driver = {
	name:		"ttyS",
	write:		s3c3410_console_write,
	device:		s3c3410_console_device,
	setup:		s3c3410_console_setup,
	flags:		CON_PRINTBUFFER,
	index:		-1,
};

/* static void init_console(struct s3c3410_serial *info) */
static int s3c3410_console_setup(struct console *c, char *p)
{
	struct s3c3410_serial *info = &s3c3410_info;
	
	memset(info, 0, sizeof(struct s3c3410_serial));
	info->use_ints = 0;
	info->cts_state = 1;
	info->is_cons = 1;
//	info->usart->cr = 0; 
...
//     info->usart->ier = 0;
	...
	s3c3410_console_initialized = 1;
	return 0;
}

static kdev_t s3c3410_console_device(struct console *c)
{
	return MKDEV(TTY_MAJOR, 64 + c->index);
}

static void s3c3410_console_write (struct console *co, const char *str,
			   unsigned int count)
{
#if defined(CONFIG_CONSOLE_NULL)
	return;
#else
	static struct s3c3410_serial *info = &s3c3410_info;
	
	if (!s3c3410_console_initialized)
...
void __init s3c3410_console_init(void)
{
	register_console ( &s3c3410_driver );
}
