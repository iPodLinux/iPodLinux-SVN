...
#define ATAG_HWADDR (ATAG_CORE+50) /* FIXME: Unreserved */
...
static int __init parse_tag_hwaddr(const struct tag *tag)
{
	char *dest;
	int i;
	struct atag_hwaddr *hwaddr = (struct atag_hwaddr *) &tag->u.core;

	if (!strcmp("eth0", hwaddr->name))
		dest = S3C2500_HWADDR0;
	else if (!strcmp("eth1", hwaddr->name))
		dest = S3C2500_HWADDR1;
	else return 0;

	for (i = 0; i < 5; i++)
		dest += sprintf(dest, "%02x:", hwaddr->hwaddr[i]);
	sprintf(dest, "%02x", hwaddr->hwaddr[i]);

	return 0;
}

__tagtable(ATAG_HWADDR, parse_tag_hwaddr);
