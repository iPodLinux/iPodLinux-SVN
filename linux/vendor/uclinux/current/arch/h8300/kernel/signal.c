...
struct sigframe
{
	unsigned long dummy_signo;
	unsigned long dummy_vector;
#if defined(CONFIG_CPU_H8S)
	unsigned short exr;
#endif
	unsigned long dummy_handler;
	char *pretcode;
			 ...
			 sigset_t *set, struct pt_regs *regs)
{
	...
	regs->er0 = (current->exec_domain
		     ...
		     : sig);
	regs->er1 = frame->psc; /* secret second arg to signal handlers */
			    ...
			    sigset_t *set, struct pt_regs *regs)
{
	...
	regs->er0 = (current->exec_domain
		     ...
		     ? current->exec_domain->signal_invmap[sig]
		     : sig);
	regs->er1 = frame->pinfo;  /* second arg to sigaction and friends */
	regs->er2 = frame->puc;    /* third arg to sigaction and friends */
...
asmlinkage int do_signal(sigset_t *oldset, struct pt_regs *regs)
{
	...
	for (;;) {
		...
		if ((current->ptrace & PT_PTRACED) && signr != SIGKILL) {
			...
			/* DAVIDM do we need to clear trace bit in CCR */
