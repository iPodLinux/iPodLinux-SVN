...
void handle_exception (struct gdb_regs *regs)
{
	...
	*ptr++ = ':';
	ptr = mem2hex((char *)&regs->cp0_epc, ptr, 8, 0);
	*ptr++ = ';';
	...
	*ptr++ = ':';
	ptr = mem2hex((char *)&regs->reg30, ptr, 8, 0);
	*ptr++ = ';';
	...
	*ptr++ = ':';
	ptr = mem2hex((char *)&regs->reg29, ptr, 8, 0);
	*ptr++ = ';';
...
#include <asm/sibyte/board.h>
		...
		case 'g':
			ptr = output_buffer;
			ptr = mem2hex((char *)&regs->reg0, ptr, 32*8, 0); /* r0...r31 */
			ptr = mem2hex((char *)&regs->cp0_status, ptr, 6*8, 0); /* cp0 */
			ptr = mem2hex((char *)&regs->fpr0, ptr, 32*8, 0); /* f0...31 */
			ptr = mem2hex((char *)&regs->cp1_fsr, ptr, 2*8, 0); /* cp1 */
			ptr = mem2hex((char *)&regs->frame_ptr, ptr, 2*8, 0); /* frp */
			ptr = mem2hex((char *)&regs->cp0_index, ptr, 16*8, 0); /* cp0 */
			break;
		...
		{
			ptr = &input_buffer[1];
			hex2mem(ptr, (char *)&regs->reg0, 32*8, 0);
			ptr += 32*16;
			hex2mem(ptr, (char *)&regs->cp0_status, 6*8, 0);
			ptr += 6*16;
			hex2mem(ptr, (char *)&regs->fpr0, 32*8, 0);
			ptr += 32*16;
			hex2mem(ptr, (char *)&regs->cp1_fsr, 2*8, 0);
			ptr += 2*16;
			hex2mem(ptr, (char *)&regs->frame_ptr, 2*8, 0);
			ptr += 2*16;
			hex2mem(ptr, (char *)&regs->cp0_index, 16*8, 0);
