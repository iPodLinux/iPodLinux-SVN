...
CFLAGS := -mno-underscore -fno-builtin -DNO_CACHE $(CFLAGS) -pipe -DNO_MM -DNO_FPU -DNO_CACHE -D__ELF__ -DMAGIC_ROM_PTR -DNO_FORGET -DUTS_SYSNAME='"uClinux"' -D__linux__ 
AFLAGS := $(AFLAGS) -pipe -DNO_MM -DNO_FPU -DNO_CACHE -D__ELF__ -DMAGIC_ROM_PTR -DUTS_SYSNAME='"uClinux"' -Wa

ifndef CONFIG_FULLDEBUG
LINKFLAGS = -T $(TOPDIR)/arch/$(ARCH)/platform/$(PLATFORM)/$(BOARD)/$(MODEL).ld
else
LINKFLAGS = -S -T $(TOPDIR)/arch/$(ARCH)/platform/$(PLATFORM)/$(BOARD)/$(MODEL).ld
endif
...
archclean:
	@$(MAKEBOOT) clean
	rm -f arch/$(ARCH)/platform/$(PLATFORM)/frio_defs.h
