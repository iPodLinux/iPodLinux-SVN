...
static void init_config_rom(struct ti_ipod *ipod)
{
	...
	ipod->host->csr.guid_hi = 0x00000000;
	ipod->host->csr.guid_lo = 0x024f3638;
	ipod->host->csr.cyc_clk_acc = 0xff; // TODO: LYNX_CYCLE_TIMER
	ipod->host->csr.max_rec = 6;
	ipod->host->csr.lnk_spd = 2;
}
...
static void ipod_1394_set_hw_config_rom(struct hpsb_host *host, quadlet_t *config_rom)
{
	int i;
	int config_rom_length = host->csr.rom->cache_head->len / 4;

	/*
         * set AR_CSR_Size and CSR_Size (multiplies by 4 to get bytes)
	 */
	fw_reg_write(LYNX_CFR_CTRL, config_rom_length << 18 | config_rom_length << 2);
	...
	for (i = 0; i < config_rom_length; i++) {
		/* log data register, write the ROM data */
		fw_reg_write(LYNX_LOG_ROM_DATA, be32_to_cpu(config_rom[i]));
	}
...
static struct hpsb_host_driver ipod_1394_hpsb_driver = {
	.owner = THIS_MODULE,
	.name = IPOD_1394_DRIVER_NAME,
	.set_hw_config_rom = ipod_1394_set_hw_config_rom,
...
static int ipod_1394_interrupt(int irq, void *dev_id, struct pt_regs *regs_are_unused)
{
...
static struct platform_device ipod_1394_device = {
	.name = IPOD_1394_DRIVER_NAME,
/*
	.probe = ipod_1394_probe,
*/
	// .remove = ipod_1394_remove,
	// .suspend = ipod_1394_suspend,
	// .resume = ipod_1394_resume
};

/*
{
	driver_register(&ipod_1394_device);
}
*/
...
static int __init ipod_1394_init(void)
{
	...
	platform_device_register(&ipod_1394_device);

	ipod_host = hpsb_alloc_host(&ipod_1394_hpsb_driver, sizeof(struct ti_ipod), &ipod_1394_device.dev);
	if (!ipod_host) {
...
static void __exit ipod_1394_exit(void)
{
	...
	platform_device_unregister(&ipod_1394_device);
