...
static void ipod_update_display(struct fb_info *p, int sx, int sy, int width, int height)
{
	int cursor_pos;
	int y;
	int mx = sx + width;
	...
	unsigned char *addr = p->screen_base;
	...
	for (y = sy; y <= my; y++) {
		...
		img_data = addr + (sx<<1) + (y * p->fix.line_length);
...
static struct fb_fix_screeninfo ipodfb_fix __initdata = {
	.id =           "iPod",
	.type=          FB_TYPE_PACKED_PIXELS,
	.visual =       FB_VISUAL_PSEUDOCOLOR,	/* fixed visual */
	.xpanstep =     0,
	.ypanstep =     0,
        .ywrapstep =    0,
	.accel =        FB_ACCEL_NONE,
...
static struct fb_info ipodfb_info;

static struct fb_var_screeninfo ipodfb_var __initdata = {
	.xres =         IPOD_LCD_WIDTH,
	.yres =         IPOD_LCD_HEIGHT,
	.xres_virtual = IPOD_LCD_WIDTH,
	.yres_virtual = IPOD_LCD_HEIGHT,
	.bits_per_pixel = 2,
	.grayscale =    1,
};

static int ipodfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info);
static int ipodfb_blank(int blank, struct fb_info *info);
static void ipodfb_fillrect(struct fb_info *p, const struct fb_fillrect *region);
static void ipodfb_copyarea(struct fb_info *p, const struct fb_copyarea *area);
static void ipodfb_imageblit(struct fb_info *p, const struct fb_image *image);
static int ipodfb_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg, struct fb_info *info);

static struct fb_ops ipodfb_ops = {
	.owner =	THIS_MODULE,
	.fb_check_var =	ipodfb_check_var,
	.fb_blank =	ipodfb_blank,
	.fb_fillrect =	ipodfb_fillrect,
	.fb_copyarea =	ipodfb_copyarea,
	.fb_imageblit =	ipodfb_imageblit,
	.fb_cursor =	soft_cursor,
	.fb_ioctl =	ipodfb_ioctl,
};
...
static void ipodfb_fillrect(struct fb_info *p, const struct fb_fillrect *region)
{
	...
	ipod_update_display(p, region->dx, region->dy, region->width, region->height);
}
...
static void ipodfb_copyarea(struct fb_info *p, const struct fb_copyarea *area)
{
	...
	ipod_update_display(p, area->dx, area->dy, area->width, area->height);
}

static void ipodfb_imageblit(struct fb_info *p, const struct fb_image *image)
{
	cfb_imageblit(p, image);
	ipod_update_display(p, image->dx, image->dy, image->width, image->height);
}
...
{
#if 0
	ipodfb_info.smem_start = (unsigned long)ipod_scr;
	ipodfb_info.smem_len = IPOD_LCD_HEIGHT * (IPOD_LCD_WIDTH/4);
#endif
	ipod_scr = kmalloc(IPOD_LCD_HEIGHT * (IPOD_LCD_WIDTH/4), GFP_KERNEL);

	ipodfb_info.screen_base = ipod_scr;
	ipodfb_info.fbops = &ipodfb_ops;
	ipodfb_info.fix = ipodfb_fix;
	ipodfb_info.fix.line_length =  IPOD_LCD_WIDTH >> 2;	/* cfb2 default */
	ipodfb_info.var = ipodfb_var;
	ipodfb_info.flags = FBINFO_FLAG_DEFAULT;

	/* This has to been done !!! */
	fb_alloc_cmap(&ipodfb_info.cmap, 4, 0);

	init_lcd();

	if (register_framebuffer(&ipodfb_info) < 0) {
		return -EINVAL;
	}

	printk(KERN_ERR "fb%d: %s frame buffer device\n", ipodfb_info.node, ipodfb_info.fix.id);

	return 0;
}

static void __exit ipodfb_exit(void)
{
	unregister_framebuffer(&ipodfb_info);
	kfree(ipod_scr);
}

int __init
ipodfb_setup(char *arg)
{
...
#ifdef MODULE
module_init(ipodfb_init);
#endif
module_exit(ipodfb_exit);
...
static int __init ipodfb_init(char *options)
{
...
static void __exit ipodfb_exit()
{
	/*
...
MODULE_LICENSE("GPL");
MODULE_AUTHOER("Bernard Leach <leachbj@bouncycastle.org>");
MODULE_DESCRIPTION("Framebuffer driver for iPod LCD");
