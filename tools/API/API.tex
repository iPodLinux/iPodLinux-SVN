...
in the window stack if \verb|whence| is \verb|TTK_MOVE_REL|.
...
If the ``window'' is set to a directive like \verb|TTK_MENU_DONOTHING| (as explained below) our of
e.g.~an error that might not be permanent, you can set \verb|item->flags &= ~TTK_MENU_MADESUB| from
your menu handler, and it will be called again the next time someone selects the item.
...
\item[+ Handler returned directive but should still be called every time.] Set \verb|item->flags &= ~TTK_MENU_MADESUB| before returning from the handler.
\end{description}
